...
export class UserService {
  ...
  attemptVerify(userId: string, verifyCode: string): Observable<any> {
    return this.apiService
      .put('/users/verify', { user_id: userId, verify_code: verifyCode })
      .pipe(map((data) => data.response));
  }
  ...
  updatePassword(oldPass: string, newPass: string): Observable<any> {
    return this.apiService
      .put('/users/password', { old_pass: oldPass, new_pass: newPass })
      .pipe(map((data) => data.response));
  }
  ...
  populate(): void {
    this.apiService
      ...
      .subscribe(
        (data) =>
          typeof data === 'object' ? this.setAuth(data) : this.purgeAuth(),
        (err) => this.purgeAuth()
      );
  }
}
